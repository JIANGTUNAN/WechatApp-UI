<template>
	<view>
		...
		<scroll-view scroll-x="true" class="scroll-box">
			<view :class="'item-box'+' bg'+((index+'').slice(-1))" v-for="(item,index) in data" :key="index"
				@click="chooseItem(item)">
				<image :src="item.src" :style="'width:'+imgWidth+'rpx;'+'height:'+imgHeight+'rpx;'" mode=""></image>
				<view class="item-name">{{item.name}}</view>
				<view class="item-descr">{{item[textKey]}}</view>
			</view>
		</scroll-view>
	</view>
</template>
