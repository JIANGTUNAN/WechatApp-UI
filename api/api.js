...
import produstList from '@/util/products.js';
...
// 根据店铺id获取产品
export function getProduct(storeId) {
	return new Promise((res, rej) => {
		const filterData = produstList.filter(i => i.storeId == storeId).sort((a, b) => b.sort - a.sort);
		res({
			code: 200,
			data: filterData
		})
